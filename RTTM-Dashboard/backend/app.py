...
if os.path.abspath(sys.executable).lower() != os.path.abspath(venv_python).lower():
    if os.path.exists(venv_python):
        print("🔄 Detected global Python. Auto-switching to the project's virtual environment...")
        ...
        print("⚠️ Warning: Virtual environment not found at 'venv'. Running anyway...")
...
if os.environ.get('WERKZEUG_RUN_MAIN') != 'true':
    ...
    if os.path.exists(frontend_dir):
        print("🚀 Starting React Frontend in the background...")
...
def manage_keywords():
    ...
    if request.method == 'POST':
        ...
        try:
            cursor = conn.cursor()
            cursor.execute('INSERT INTO keywords (keyword) VALUES (?)', (data['keyword'],))
            conn.commit()
